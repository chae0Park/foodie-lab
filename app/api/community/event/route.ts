...
import { Event } from '@prisma/client'; // Prisma에서 생성된 타입
...
export async function POST(req: NextRequest):Promise<NextResponse> {
  ...
  const authorizationHeader:(string | null) = req.headers.get('Authorization');
    ...
    try {
      ...
      const { title, time, address, fee, instructions, estimatedTime, imageBase64 } = body;
      console.log('community-body의 값은?', body);
      // Validation
      if (
        isInvalidText(title) ||
        isInvalidText(time) ||
        isInvalidText(address) ||
        isInvalidText(fee) ||
      ...
      return NextResponse.json({ message: 'Meal shared successfully!', newEvent }, {status: 200});
